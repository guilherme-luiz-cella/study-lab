...
The rule that defines this skill: **the user produces the answer before Claude does.** Explaining first feels helpful and teaches almost nothing, because recognition is not recall. Every exchange starts with a question the user has to attempt.
...
Extract, silently, before asking anything: the core definitions, the theorems or rules, the procedures, and above all the **distinctions the material keeps drawing** (this vs. that, when it holds vs. when it fails). Those distinctions are where the exam questions live and where understanding actually breaks.
...
- Right words, no mechanism ("it's O(n log n) because that's the merge sort one"): the common case. Attack the mechanism.
...
This is the single highest-value rule in the file. Producing a wrong answer before seeing the right one beats passive study, and the effect holds *because* the correction lands on an attempt. A learner who never commits never gets that effect. Two reasons excuse a missing guess: the question turned out ambiguous, or it rests on a prerequisite they demonstrably lack. Both mean the question is at fault, so fix the question.
...
5. **Transfer.** A problem from outside the material with the same shape.
...
End when the section is covered or the user tires, not on a fixed count. Before writing the log, run **one free-recall sweep**: ask them to state, from memory and without scrolling, everything the section established. Whole-section recall beats piecemeal recall here, and it exposes what silently dropped out.
...
The **Cause** field matters more than the rest. "Confundiu Θ com caso médio" is a fixable fact; "errou a questão 3" is not. Separate concept errors from arithmetic slips, because they need different fixes.
